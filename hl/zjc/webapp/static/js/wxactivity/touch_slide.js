/*Copyright:成都零点信息技术有限公司QQ:863230477*/
/*使用方法：
var slider = new zySlide(selector,dir,endFun,lock,transEnd);
*/
var $support = {
...
zySlide.prototype = {
	...
	refresh: function() {
		...
		// setting distance
		if(self.dir=="H")
			self.distance = parseInt(conf.distance || document.body.clientWidth);
		else
			self.distance = parseInt(conf.distance || document.body.clientHeight);
		// setting maxX maxY
	...
	moveToPoint: function(point,time) {
		var self = this;
		self.currentPoint =
			(point < 0) ? 0 :
			...
			parseInt(point);
		if(time || time==0){
            self.element.style.webkitTransitionDuration = time+'ms';
		}else{
