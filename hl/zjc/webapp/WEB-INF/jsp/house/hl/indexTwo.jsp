  ...
  <div class="container bgf2"> 
	...
	<div class="main_slid">
	 <c:forEach items="${goods}" varStatus="i" var="item"> 
	  ...
	  <!-- 规格数量 -->
	<div class="itb">
		<p class="itb_text font50 color333 center"> </p>
	    <div class="itb_input font50 color333">
		  数量:<input type="text" placeholder="请输入数量" class="val_text"/>
		</div>
		<p class="dan_text right color333 font45 border_bottom">单位:米</p>
		<div class="btn_ok mgauto">
			<button>确定</button>
		</div>	
		</div>
	...
	function type(type_id){
		window.location.href = "hltypeIndexs.jhtml?type_id="+type_id+"&user_id="+${user_id};
	}
	function order(){
		var order=$(".divs").text();
	...
	function getGoodsVOByMaterial_name(id){
		window.location.href = "getGoodsVOByMaterial_name.jhtml?id="+id;
	}
	function sousuo(){
