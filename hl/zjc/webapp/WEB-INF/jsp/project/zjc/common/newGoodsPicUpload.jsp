	...
	$("#picture").on("click", function() {
		...
		html2canvas(img_lg, {
			...
			onrendered : function(canvas) {
				...
				newImg.src = dataUrl;
				imagesAjax(dataUrl)
				window.parent.Ext.getCmp('${windowsId}').hide();
	...
	function imagesAjax(src) {
		...
		$.ajax({
			...
			success : function(data) {
				var flag = "${updategoods}";
				var updateinputName = "${inputName}";
				if(flag != ""){
					updateinputName=updateinputName.replace('_update','');
              		$('input[name="'+updateinputName+'"]', parent.document).val(data.path);
              		parent.document.getElementById("showImage-update").src = data.path;
				}else{
					$('input[name="${inputName}"]', parent.document).val(data.path);
					 parent.document.getElementById("showImage").src = data.path;
				}
               parent.document.getElementById("${inputName}").src = data.path;
			}
		}); 
	}
	</script>
