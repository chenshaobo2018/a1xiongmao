...
<aos:html title="商品终审" base="http" lib="ext">
	...
	<script type="text/javascript">
		function w_approval_goods_show(){
			Ext.getCmp('w_approval_goods').show();
		}
	</script>
	<aos:body>
		<!-- UEditer -->
...
<aos:onready>
	<aos:viewport layout="fit">
		<aos:panel layout="anchor" width="auto" border="false">
			<aos:formpanel id="f_query" layout="column" header="false" border="false" anchor="100%">
				...
				<aos:combobox name="cat_id" fieldLabel="所有分类" url="zjcGoodsCategoryService.listPositionComboBoxDataByParam&parent_id=0" columnWidth="0.25" />
				<aos:combobox name="goods_state_2" fieldLabel="审核状态"  dicField="goods_audit_status" columnWidth="0.25" />
				<aos:docked dock="bottom" ui="footer" margin="0 0 8 0">
					<aos:dockeditem xtype="tbfill" />
					<aos:dockeditem xtype="button" text="查询" onclick="g_approval_goods_query" icon="query.png" />
					<aos:dockeditem xtype="button" text="重置" onclick="AOS.reset(f_query);" icon="refresh.png" />
					<%-- <aos:dockeditem xtype="button" text="缓存详情" onclick="cacheGoodsDetail" icon="add2.png" /> --%>
					<aos:dockeditem xtype="tbfill" />
				</aos:docked>
			</aos:formpanel>
		
			<aos:gridpanel id="g_approval_goods" url="zjcGoodsService.listApprovalGoods" autoLoad="true" height="750" anchor="100%" border="1 0 1 3">
				<aos:menu>
					<aos:menuitem text="刷新" onclick="#g_approval_goods_store.reload();" icon="refresh.png" />
				</aos:menu>
				...
				<aos:column header="店铺名称" dataIndex="store_name" />
				...
				<%-- <aos:column header="比例易支付" dataIndex="market_price" celltip="true" /> --%>
				...
				<aos:column header="审核状态" dataIndex="goods_state_2" celltip="true" rendererField="goods_audit_status"/>
				<aos:column header="操作" rendererFn="fn_button_render" align="center" fixedWidth="60" />
			</aos:gridpanel>
		</aos:panel>
	
	<aos:window id="w_approval_goods" title="商品审核" width="1100" height="600" layout="fit" autoScroll="true" onshow="w_approval_goods_selection" onclose="closeset">
		<aos:tabpanel id="id_tabs" region="center" tabPosition="top" bodyBorder="0 0 0 0" margin="0 0 2 0">
			<aos:tab title="通用信息" onshow="setGrey">
				<aos:formpanel id="f_goods_info" width="1100" layout="column" labelWidth="150"  msgTarget="qtip">
					...
					<aos:textfield fieldLabel="拼团价格(现金)" name="shop_group_price" columnWidth="0.3" readOnly="true"/>
					<aos:textfield fieldLabel="拼团价格(易物券)" name="shop_group_market_price" columnWidth="0.3" readOnly="true"/>
					<aos:textfield fieldLabel="需要几人拼团" name="shop_group_person" columnWidth="0.3" readOnly="true"/>
					...
					<aos:radioboxgroup fieldLabel="是否vip" columns="4" columnWidth="0.3">
						...
						<aos:radiobox name="is_vip" boxLabel="否" inputValue="0" />
					</aos:radioboxgroup>
					<%-- <aos:radioboxgroup fieldLabel="是否xpt" columns="4" columnWidth="0.3">
						<aos:radiobox name="xpt" boxLabel="是" inputValue="1" />
						<aos:radiobox name="xpt" boxLabel="否" inputValue="0" />
					</aos:radioboxgroup> --%>
					
					<%-- <aos:radioboxgroup fieldLabel="限时抢购" columns="8" columnWidth="0.5" onchange="isLimitBuy" >
						<aos:radiobox name="prompt_goods" boxLabel="是" inputValue="1" />
						<aos:radiobox name="prompt_goods" boxLabel="否" inputValue="0" />
					</aos:radioboxgroup> --%>
					...
					<aos:fieldset title="限时抢购明细" labelWidth="120" columnWidth="1" border="true" id="limit_buy">
						<%-- <aos:datetimefield name="start_time" id="start_time" editable="false" fieldLabel="限时开始时间" columnWidth="0.42"/>
						<aos:datetimefield name="end_time" id="end_time" editable="false" fieldLabel="限时结束时间" columnWidth="0.43"/> --%>
						<aos:datefield name="activity_date" id="activity_date" editable="false" fieldLabel="活动日期" columnWidth="0.42" />
						<aos:combobox name="activity_date_part" id="activity_date_part" fieldLabel="活动时间段" dicField="limit_activity_part" columnWidth="0.43" />
						<aos:textfield fieldLabel="原在线支付" name="limit_cost_price" id="limit_cost_price" columnWidth="0.42" />
						<aos:numberfield fieldLabel="原易支付" name="limit_market_price" id="limit_market_price" minValue="0"  columnWidth="0.43" />
					</aos:fieldset>
					<aos:fieldset title="商品审核" labelWidth="70" columnWidth="1" border="true">
						<aos:radioboxgroup fieldLabel="审核" columns="4" columnWidth="0.85">
							<aos:radiobox name="goods_state_2" boxLabel="通过" inputValue="3" />
							<aos:radiobox name="goods_state_2" boxLabel="不通过" inputValue="2" />
						</aos:radioboxgroup>
						<aos:textareafield fieldLabel="备注" name="goods_state_2_mark" id="goods_state_2_mark" columnWidth="0.85" />
					</aos:fieldset>
				</aos:formpanel>
			</aos:tab>
			<aos:tab title="商品详情" onshow="updateOnshow">
			...
			<aos:tab title="商品规格" layout="anchor" onshow="romove_ash">
				<aos:formpanel id="f_goods_spe" layout="column" labelWidth="100" margin="5" anchor="100%" border="true" >
					...
					<%-- <aos:combobox name="type_id" fieldLabel="商品类型" labelWidth="60" url="zjcGoodsTypeService.listTypeComboBoxData" columnWidth="0.55"/> --%>
		...
		<aos:docked dock="bottom" ui="footer">
			<aos:dockeditem xtype="tbfill" />
			<aos:dockeditem onclick="f_approval_goods_submit" text="保存" icon="ok.png" id="save_btn"/>
			<aos:dockeditem onclick="#w_approval_goods.close();" text="关闭" icon="close.png" />
		</aos:docked>
	</aos:window>
	...
	<script type="text/javascript">
		...
		function closeset(){
		 ...
		 //查询参数列表
	    function g_approval_goods_query() {
	    	var params = AOS.getValue('f_query');
	    	g_approval_goods_store.getProxy().extraParams = params;
	        g_approval_goods_store.loadPage(1);
	    }
		...
		//审核商品后，缓存商品详细内容到redis
		function cacheGoodsDetail(){
			AOS.ajax({
                url: 'zjcGoodsService.cacheGoodsDetail',
                ok: function (data) {
                	AOS.tip(data.appmsg);
                }
            });
		...
		function fn_button_render(value, metaData, record, rowIndex, colIndex,store) {
			return '<input type="button" value="审核" class="cbtn" onclick="w_approval_goods_show();" />';
		}
		/* 弹出窗口监听 */
		function w_approval_goods_selection(){
			var record = AOS.selectone(g_approval_goods);
			f_goods_info.form.reset();
            AOS.ajax({
            	params : {
            		goods_id: record.data.goods_id
            	},
                url: 'zjcGoodsService.getZSGoodsInfo',
                ok: function (data) {
                	console.log(data);
                	...
                	//刷新备注
                	if(data.admin_aduit_info == undefined || data.admin_aduit_info==''){
                		$("#goods_state_2_mark-inputEl").val("");
                	}else{
                		var strs = new Array();
                    	strs = data.admin_aduit_info.split(";");
                    	$("#goods_state_2_mark-inputEl").val(strs[3].split(":")[2].replace(/\"/g,"")); 
                	}
		...
		function f_approval_goods_submit(){
			...
			if(is_limit == 1){
				var activity_date = Ext.getCmp('activity_date').value;
				var activity_date_part = Ext.getCmp('activity_date_part').value;
				console.log(activity_date+","+activity_date_part);
				if(activity_date == null || activity_date == ''){
					 AOS.warn("请选择活动时间");
					 return;
				}
				if(activity_date_part == null || activity_date_part == ''){
					 AOS.warn("请选择商品的活动时间段");
					 return;
				}
				var limit_market_price = Ext.getCmp('limit_market_price').value;
				var limit_cost_price = Ext.getCmp('limit_cost_price').value;
				if(limit_cost_price == null || limit_cost_price == ''){
					 AOS.warn("请填写商品的折扣价格（现金）");
					 return;
				}
				if(limit_market_price == null || limit_market_price == ''){
					 AOS.warn("请填写商品的折扣价格（券）");
					 return;
				}
			} 
			AOS.ajax({
                forms: f_goods_info,
                url: 'zjcGoodsService.updateApprovalGoods',
                ok: function (data) {
                	if(data.appcode == '1'){
                		w_approval_goods.close();
                		g_approval_goods_store.reload();
                        AOS.tip(data.appmsg);
                	}else{
		...
		function changeRate(me,newValue){
			...
			} else if(newValue.commodity_categories == 2){//热销预售
		...
		//监控是否选择限时抢购
		function isLimitBuy(me,newValue){
			is_limit = newValue.prompt_goods;
			if(is_limit == 1){//限时商品
				$("#limit_buy").show();
			} else {//不限时商品
				$("#limit_buy").hide();
