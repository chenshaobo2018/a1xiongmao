...
<aos:onready>
	...
	<aos:panel layout="auto" autoScroll="true" border="false">
		<aos:panel layout="anchor" width="auto" border="false">
			<aos:formpanel id="f_query" layout="column" header="false" border="false" anchor="100%" >
				...
				<aos:textfield name="goods_name" fieldLabel="商品名称"  columnWidth="0.25" />
				...
				<aos:combobox name="is_car" fieldLabel="特殊商品" dicField="is_car" columnWidth="0.25" />
			...
			<aos:gridpanel id="g_goods" url="zjcGoodsService.listGoods" forceFit="true" autoLoad="true" 
			height="750" anchor="100%">
				<aos:menu>
					<aos:menuitem text="修改" onclick="w_goods_u_show" icon="edit.png" />
					<%-- <aos:menuitem text="删除" onclick="g_goods_del" icon="del.png" /> --%>
					<aos:menuitem xtype="menuseparator" />
					<aos:menuitem text="刷新" onclick="#g_goods_store.reload();" icon="refresh.png" />
				</aos:menu>
				<aos:docked forceBoder="0 0 1 0" >
					<aos:dockeditem onclick="w_goods_u_show" text="修改" icon="edit.png" />
					<%-- <aos:dockeditem onclick="g_goods_del" text="删除" icon="del.png" /> --%>
				</aos:docked>
				<aos:plugins>
	...
	<aos:window id="w_goods_u" title="修改商品" width="1100" maxHeight="750" layout="fit" autoScroll="true"  onclose="closeset">
		<aos:tabpanel id="id_tabs_u" region="center" tabPosition="top" bodyBorder="0 0 0 0" margin="0 0 2 0">
			<aos:tab title="通用信息" >
				<aos:formpanel id="f_goods_info_u" width="650" layout="column" labelWidth="150" msgTarget="qtip">
					...
					<aos:textfield fieldLabel="商品名称" name="goods_name" columnWidth="0.9" />
					...
					<aos:textfield fieldLabel="滴支付" name="drops" columnWidth="0.3" />
					<aos:textfield fieldLabel="库存" name="store_count" columnWidth="0.3" />
					<aos:textfield fieldLabel="销量" name="sales_sum" columnWidth="0.3" />
					<aos:textfield fieldLabel="今日每人限购次数" name="today_limit_times" columnWidth="0.3" allowBlank="false"/>
					<aos:textfield fieldLabel="每人每次限购数量" name="today_limit_nums" columnWidth="0.3" allowBlank="false"/>
					<aos:textfield fieldLabel="预计发货时间" name="expect_delivery" columnWidth="0.3" emptyText="预计几天内发货" allowBlank="false"/>
					...
					<%-- <aos:radioboxgroup fieldLabel="特价" columns="4" columnWidth="0.3">
			...
			<aos:tab title="商品规格" layout="anchor" height="400">
				<aos:formpanel id="f_goods_spe" layout="column" labelWidth="100" margin="5" anchor="100%" border="true">
					<aos:combobox name="type_id" fieldLabel="商品类型" labelWidth="60" url="zjcGoodsTypeService.listTypeComboBoxData" columnWidth="0.55" onchange="typeOnchange"/>
		...
		<aos:docked dock="bottom" ui="footer">
			<aos:dockeditem xtype="tbfill" />
			<aos:dockeditem onclick="f_goods_u_submit" text="保存" icon="ok.png" />
	...
	<script type="text/javascript">
		...
		function free_shipping(me,newValue){
			...
			if(is_free == 1){//包邮
				$("#postage1").hide();
				f_goods_info_u.down("[name='postage']").setValue('0');
			} else {//不包邮
				$("#postage1").show();
			}
		}
	    ...
	    function free_render(){
			is_free = f_goods_info_u.down("[name='is_free_shipping']").getValue();
			if(is_free == 1){//包邮
				$("#postage1").hide();
			} else {//不包邮
				$("#postage1").show();
		...
		//商品添加
		/* function f_goods_save(){
			AOS.ajax({
                forms: f_goods_info,
                url: 'zjcGoodsService.saveGoods&store_id=11',
                ok: function (data) {
                	if(data.appcode == '1'){
                		w_goods.hide();
                		g_goods_store.reload();
	  ...
	  function fn_beforeedit(obj, e) {
		...
		function fn_export_excel(){
			...
			AOS.file('do.jhtml?router=zjcGoodsService.exportgoodsInfo&juid=${juid}&user_id='+params.user_id +'&store_name='+encodeURI(encodeURI(params.store_name)) +'&goods_name='+encodeURI(encodeURI(params.goods_name)) 
					+ '&is_on_sale=' + params.is_on_sale +'&is_new=' + params.is_new +'&is_car=' + params.is_car + '&cat_id=' + params.cat_id
					+ '&bind_id=' + params.bind_id);
		}
		
		//改变支付类别调整比例
		function changeRate(me,newValue){
			if(newValue.commodity_categories == 1){
				f_goods_info_u.down('[name=goods_weight]').setValue('0');
				f_goods_info_u.down('[name=store_rebate_rate]').setValue('0');
				f_goods_info_u.down('[name=is_hot]').setValue('0');
				f_goods_info_u.down('[name=is_mixed]').setValue('0');
				f_goods_info_u.down('[name=is_car]').setValue('0');
			} else if(newValue.commodity_categories == 2){
				f_goods_info_u.down('[name=goods_weight]').setValue('0');
				f_goods_info_u.down('[name=store_rebate_rate]').setValue('1');
				f_goods_info_u.down('[name=is_hot]').setValue('1');
				f_goods_info_u.down('[name=is_mixed]').setValue('1');
				f_goods_info_u.down('[name=is_car]').setValue('1');
			} else if(newValue.commodity_categories == 3){
				f_goods_info_u.down('[name=goods_weight]').setValue('0');
				f_goods_info_u.down('[name=store_rebate_rate]').setValue('1');
				f_goods_info_u.down('[name=is_hot]').setValue('0');
				f_goods_info_u.down('[name=is_mixed]').setValue('0');
