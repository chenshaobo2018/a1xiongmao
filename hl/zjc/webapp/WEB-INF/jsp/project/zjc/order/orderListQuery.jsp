...
<aos:onready>
	<aos:viewport layout="border">
		<aos:formpanel id="f_query" layout="column" labelWidth="150" header="false" region="north" border="false">
			...
			<aos:combobox name="pay_status" value="1" fieldLabel="支付状态" emptyText="支付状态" dicField="pay_status" columnWidth="0.33" />
			<aos:combobox name="pay_code" fieldLabel="支付方式"  emptyText="支付方式" dicField="pay_code" columnWidth="0.33" />
			<aos:combobox name="shipping_status" value="0" fieldLabel="发货状态" emptyText="发货状态" dicField="shipping_status" columnWidth="0.33" />
		...
		<aos:gridpanel id="g_order"  url="zjcOrderService.listZjcOrders" region="center">
			...
			<aos:column header="商家ID" dataIndex="mj" />
			<aos:column header="订单编号" dataIndex="order_sn" celltip="true"/>
			...
			<aos:column header="现金" dataIndex="cash"/>
			<aos:column header="券" dataIndex="barter_coupons" rendererFn="praseToInt"/>
			<aos:column header="支付状态" dataIndex="pay_status" rendererField="pay_status" />
			<aos:column header="订单状态" dataIndex="order_status" rendererField="order_status" />
			<aos:column header="支付方式" dataIndex="pay_code" rendererField="pay_code_name"/>
			...
			<aos:column header="发货时间" dataIndex="shipping_time" rendererFn="format_shippingTime" celltip="true"/>
			<aos:column header="收货时间" dataIndex="confirm_time" rendererFn="format_confirmTime" celltip="true"/>
		...
		<aos:window id="w_order_detail" layout="border" title="订单详情" maximized="true" autoScroll="true" onshow="order_onShow">
			<aos:formpanel id="f_order_detail" layout="column" labelWidth="70" margin="5" anchor="100%" border="false" header="false" region="north" autoScroll="true">
				...
				<aos:fieldset title="基本信息" labelWidth="70" >
					<aos:textfield name="order_id" fieldLabel="订单id" border="none" readOnly="true" columnWidth="0.33" />
		...
		<aos:window id="w_order_delivery" layout="border" title="订单发货" maximized="true" autoScroll="true" ><!-- delivery_onShow -->
			<aos:formpanel id="deliveryForm" layout="column" labelWidth="70" margin="5" anchor="100%" border="false" header="false" region="north" autoScroll="true" onrender="getOrderBasicInfo">
				...
				<aos:fieldset title="基本信息" labelWidth="70" >
					...
					<aos:textfield name="invoice_no" id="invoice_no" fieldLabel="物流单号" columnWidth="0.33" allowBlank="false" readOnly="true"/>
				</aos:fieldset>
	...
	<script type="text/javascript">
		...
		function fn_export_excel(){
			...
			if(flag){
				AOS.file('do.jhtml?router=zjcOrderService.exportOrderInfo&juid=${juid}&store_name='+encodeURI(encodeURI(params.store_name)) +'&goods_name='+encodeURI(encodeURI(params.goods_name)) +'&user_id='+params.user_id +'&order_sn='+params.order_sn
						+ '&order_status=' + params.order_status +'&pay_code=' + params.pay_code +'&pay_status=' + params.pay_status + '&shipping_status=' + params.shipping_status
						+ '&add_time_start=' + params.add_time_start + '&add_time_end=' + params.add_time_end+ '&user_id=' + params.user_id + '&mj=' + params.mj);
			}else{
				AOS.tip("筛选条件不能为空！");
			}
		}
