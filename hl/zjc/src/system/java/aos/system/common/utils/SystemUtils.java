...
public class SystemUtils {
	...
	public static String toTreeModalAllInOne(List<Dto> nodeDtos){
		...
		for (Dto nodeDto : nodeDtos) {
			...
			treeNode.setIconCls(nodeDto.getString("vector"));
	...
	public static String toTreeModalAsyncLoad(List<Dto> treeModels){
		...
		for (Dto model : treeModels) {
			...
			treeNode.setB(model.getString("parent_id_path"));
			treeNodes.add(treeNode);
		}
	 ...
	 * @return
	 */
	public static String genParentPathId(String curMaxNode, int maxValue) {
		String prefix = StringUtils.substringBeforeLast(curMaxNode, "_");
		String last = StringUtils.substringAfterLast(curMaxNode, "_");
		if (AOSUtils.isEmpty(last)) {
			throw new AOSException("树ID生成器生成节点ID参数错误");
		}
		int intLast = Integer.valueOf(last);
