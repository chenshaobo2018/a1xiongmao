...
public class CacheUserDataService {
	 ...
	 * @return
	 */
	public String loginSeller(ZjcSellerInfoPO sellerInfoPO, HttpServletRequest httpServletRequest){
		//缓存用户信息
		String juid = idService.uuid();
		UserModel userModel = new UserModel();
		//AOSUtils.copyProperties(sellerInfoPO, userModel);
		userModel.setId(1);
		userModel.setAccount(sellerInfoPO.getMobile());
		userModel.setName(sellerInfoPO.getReal_name());
		userModel.setSex(sellerInfoPO.getSex()+"");
		userModel.setStatus("1");
		userModel.setType("1");
		userModel.setOrg_id(0);
		userModel.setBiz_code("");
		userModel.setSkin("blue");
		userModel.setJuid(juid);
		AosOrgPO aosOrgPO = aosOrgDao.selectByKey(userModel.getOrg_id());
		userModel.setAosOrgPO(aosOrgPO);
		userModel.setLogin_time(AOSUtils.getDateTimeStr());
